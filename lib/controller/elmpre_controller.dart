...
class ElmPreControllerImp extends ElmPreController {
  ...
  String getShareText(int index) {
    ...
    for (int i = 0; i < elm.subtitles!.length; i++) {
      shareText.write('${elm.subtitles![i]}\n\n');
    }

    // Loop through ayahs and texts
    for (int i = 0; i < elm.ayahs!.length; i++) {
      shareText.write('${elm.ayahs![i]}\n\n');
      if (i < elm.texts!.length) {
        shareText.write('${elm.texts![i]}\n\n');
      }
    }
