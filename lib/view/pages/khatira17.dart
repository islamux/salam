...
import 'package:khatir/cubit/khatira_cubits/khatira_17_cubit.dart';
...
class Khatira17Page extends StatelessWidget {
  final int? initialPage;

  const Khatira17Page({super.key, this.initialPage});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) {
        final cubit = Khatira17Cubit();
        if (initialPage != null && initialPage! > 0) {
          cubit.goToPageAfterBuild(initialPage!);
        }
        return cubit;
      }, // Provide Khatira17Cubit
      child: Builder(
        builder: (context) {
          return Scaffold(
            appBar: AppBar(
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      final currentPageIndex =
                          context.read<Khatira17Cubit>().currentPageIndex;
                      context.read<Khatira17Cubit>().customShareContent(
                          currentPageIndex, khatiraList17NewOrder);
                    },
              ...
              leading: GestureDetector(
                onTap: () {
                  context.read<Khatira17Cubit>().resetCounter(); // Reset counter
                  // Navigator.pushNamed(
                ...
                children: [
                  Expanded(
                    child: GenericCustomTextSlider( // Changed
                      cubit: context.read<Khatira17Cubit>(),
                      dataList: khatiraList17NewOrder,
