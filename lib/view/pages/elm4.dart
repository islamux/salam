...
import 'package:khatir/cubit/elm_cubits/elm_4_cubit.dart';
...
class Elm4Page extends StatelessWidget {
  ...
  const Elm4Page({super.key, this.initialPage});

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) {
        final cubit = Elm4Cubit();
        if (initialPage != null && initialPage! > 0) {
          cubit.goToPageAfterBuild(initialPage!);
        }
        return cubit;
      }, // Provide the Elm4Cubit
      child: Builder(
        builder: (context) {
          return Scaffold(
            appBar: AppBar(
            ...
            body: SafeArea(
              child: GestureDetector( // GestureDetector can likely be removed if not needed for other purposes
                ...
                child: GenericCustomTextSlider( // Changed
                  cubit: context.read<Elm4Cubit>(),
                  dataList: elmList4NewOrder,
