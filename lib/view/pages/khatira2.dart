...
class Khatira2Page extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) {
        final cubit = Khatira2Cubit();
        if (initialPage != null && initialPage! > 0) {
          cubit.goToPageAfterBuild(initialPage!);
        }
        return cubit;
      }, // Provide the Khatira2Cubit
      child: Builder(
        builder: (context) {
          return Scaffold(
            appBar: AppBar(
                  ...
                  IconButton(
                    ...
                    onPressed: () {
                      final currentPageIndex =
                          context.read<Khatira2Cubit>().currentPageIndex;
                      context.read<Khatira2Cubit>().customShareContent(
                          currentPageIndex, khatiraList2NewOrder);
                    },
            ...
            body: SafeArea(
              child: GestureDetector( // GestureDetector can likely be removed if not needed for other purposes
                ...
                child: GenericCustomTextSlider( // Changed
                  cubit: context.read<Khatira2Cubit>(),
