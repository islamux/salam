// TextDer20eTwelve
import 'package:elm/core/data/model/elm_model.dart';
import 'package:elm/core/data/static/text/elm_text_ders_twelve.dart';

List<ElmModel> elmList12 = [
  // 1
  ElmModel(
    ayah: ElmTextDersTwelve.ayahHadithTwelveOne_1,
    text: ElmTextDersTwelve.elmtextTwelveOne_1,
  ),
  // 2
  ElmModel(
    text: ElmTextDersTwelve.elmTextTwelveTwo_1,
  ),
  // 3
  ElmModel(
    text: ElmTextDersTwelve.elmTextTwelveThree_1,
    ayah: ElmTextDersTwelve.ayahHadithTwelveThree_1,
    text2: ElmTextDersTwelve.elmTextTwelveThree_2,
    ayah2: ElmTextDersTwelve.ayahHadithTwelveThree_2,
    text3: ElmTextDersTwelve.elmTextTwelveThree_3,
  ),
  // 4
  ElmModel(
    ayah: ElmTextDersTwelve.ayahHadithTwelveFour_1,
    text: ElmTextDersTwelve.elmTextTwelveFour_1,
    ayah2: ElmTextDersTwelve.ayahHadithTwelveFour_2,
    text2: ElmTextDersTwelve.elmTextTwelveFour_2,
  ),
  // 5
  ElmModel(
    title: ElmTextDersTwelve.titleTwelveFive_1,
    subtitle: ElmTextDersTwelve.subtitleTwelveFive_1,
    text: ElmTextDersTwelve.elmTextTwelveFive_1,
    ayah: ElmTextDersTwelve.ayahHadithTwelveFive_1,
    text2: ElmTextDersTwelve.elmTextTwelveFive_2,
    ayah2: ElmTextDersTwelve.ayahHadithTwelveFive_2,
    footer: ElmTextDersTwelve.footerTwelveFive,
  ),
  // 6
  ElmModel(
    text: ElmTextDersTwelve.elmTextTwelveSix_1,
    ayah: ElmTextDersTwelve.ayahHadithTwelveSix_1,
    text2: ElmTextDersTwelve.elmTextTwelveSix_2,
    ayah2: ElmTextDersTwelve.ayahHadithTwelveSix_2,
    footer: ElmTextDersTwelve.foooterTwelveSix,
  ),
  // 7
  ElmModel(
    text: ElmTextDersTwelve.elmTextTwelveSeven_1,
    ayah: ElmTextDersTwelve.ayahHadithTwelveSeven_1,
    footer: ElmTextDersTwelve.foooterTwelveSeven,
  ),
  // 8
  ElmModel(
    title: ElmTextDersTwelve.titleTwelveEight,
    ayah: ElmTextDersTwelve.ayahHadithTwelveEight_1,
    footer: ElmTextDersTwelve.footerTwelveEight,
  ),
  // 9
  ElmModel(
    text: ElmTextDersTwelve.elmTextTwelveNine_1,
  ),
  // 10
  ElmModel(
    text: ElmTextDersTwelve.elmTextTwelveTen_1,
  ),
  // 11
  ElmModel(
    text: ElmTextDersTwelve.elmTextTwelveEleven_1,
    ayah: ElmTextDersTwelve.ayahHadithTwelveEleven_1,
  ),
  // 12
  ElmModel(
    title: ElmTextDersTwelve.titleTwelveTwelve,
    text: ElmTextDersTwelve.elmTextTwelveTwelve_1,
  ),
  // 13
  ElmModel(
      text: ElmTextDersTwelve.elmTextTwelveTherteen_1,
      ayah: ElmTextDersTwelve.ayahHadithTwelveTherteen_1,
      text2: ElmTextDersTwelve.elmTextTwelveTherteen_2),
  // 14
  ElmModel(
    title: ElmTextDersTwelve.titleTwelveFourteen,
    ayah: ElmTextDersTwelve.ayahHadithTwelveFourteen_1,
    text: ElmTextDersTwelve.elmTextTwelveFourteen_1,
    ayah2: ElmTextDersTwelve.ayahHadithTwelveFourteen_2,
    footer: ElmTextDersTwelve.footerTwelveFourteen,
  ),
  // 15
  ElmModel(
    text: ElmTextDersTwelve.elmTextTwelveFifteen_1,
    ayah: ElmTextDersTwelve.ayahHadithTwelveFifteen_1,
    text2: ElmTextDersTwelve.elmTextTwelveFifteen_2,
    ayah2: ElmTextDersTwelve.ayahHadithTwelveFifteen_2,
    text3: ElmTextDersTwelve.elmTextTwelveFifteen_3,
    footer: ElmTextDersTwelve.footerTwelveFifteen,
  ),
  // 16
  ElmModel(
    title: ElmTextDersTwelve.titleTwelveSeventeen,
    ayah: ElmTextDersTwelve.ayahHadithTwelveSixteen_1,
    text: ElmTextDersTwelve.elmTextTwelveSixteen_1,
    ayah2: ElmTextDersTwelve.ayahHadithTwelveSixteen_2,
    text3: ElmTextDersTwelve.elmTextTwelveSixteen_2,
    ayah3: ElmTextDersTwelve.ayahHadithTwelveSixteen_3,
    ayah4: ElmTextDersTwelve.ayahHadithTwelveSixteen_4,
    text2: ElmTextDersTwelve.elmTextTwelveSixteen_3,
    footer: ElmTextDersTwelve.footerTwelveSixteen,
  ),
  // 17
  ElmModel(
    title: ElmTextDersTwelve.titleTwelveSeventeen,
    text: ElmTextDersTwelve.elmTwelveSeventeen_1,
    ayah: ElmTextDersTwelve.ayahHadithTwelveSeventeen_1,
    text2: ElmTextDersTwelve.elmTwelveSeventeen_2,
    ayah2: ElmTextDersTwelve.ayahHadithTwelveSeventeen_2,
    text3: ElmTextDersTwelve.elmTwelveSeventeen_3,
    ayah3: ElmTextDersTwelve.ayahHadithTwelveSeventeen_3,
  ),
  // 18
  ElmModel(
    text: ElmTextDersTwelve.elmTwelveEighteen_1,
    text2: ElmTextDersTwelve.elmTextTwelveEighteen_2,
    ayah: ElmTextDersTwelve.ayahHadithTwelveEighteen_1,
    text3: ElmTextDersTwelve.elmTextTwelveEighteen_3,
  ),
];
