//TexDerseFourteen
import 'package:elm/core/data/model/elm_model.dart';
import 'package:elm/core/data/static/text/elm_text_ders_fourteen.dart';

List<ElmModel> elmList14 = [
  // page 1
  ElmModel(
    title: ElmTextDersFourteen.titleFourteenOne,
    text: ElmTextDersFourteen.elmTextFourteenOne_1,
  ),
  // page 2
  ElmModel(
    text: ElmTextDersFourteen.elmTextFourteenTwo_1,
    ayah: ElmTextDersFourteen.ayahHadithFourteenTwo_1,
    text2: ElmTextDersFourteen.elmTextFourteenTwo_2,
  ),
  // page 3
  ElmModel(
    ayah: ElmTextDersFourteen.ayahHadithFourteenThree_1,
    text: ElmTextDersFourteen.elmTextFourteenThree_1,
    ayah2: ElmTextDersFourteen.ayahHadithFourteenThree_2,
    text2: ElmTextDersFourteen.elmTextFourteenThree_2,
    subtitle: ElmTextDersFourteen.subtitleFourteenThree_1,
  ),
  // page 4
  ElmModel(
      text: ElmTextDersFourteen.elmTextFourteenFour_1,
      ayah: ElmTextDersFourteen.ayahHadithFourteenFour_1,
      text2: ElmTextDersFourteen.elmTextFourteenFour_2,
      ayah2: ElmTextDersFourteen.ayahHadithFourteenFour_2),
  // page 5
  ElmModel(
    text: ElmTextDersFourteen.elmTextFourteenFive_1,
    ayah: ElmTextDersFourteen.ayahHadithFourteenFive_1,
  ),
  // page 6
  ElmModel(
    text: ElmTextDersFourteen.elmTextFourteenSix_1,
    ayah: ElmTextDersFourteen.ayahHadithFourteenSix_2,
    text2: ElmTextDersFourteen.elmTextFourteenSix_2,
    ayah2: ElmTextDersFourteen.ayahHadithFourteenSix_3,
    text3: ElmTextDersFourteen.elmTextFourteenSix_3,
    ayah3: ElmTextDersFourteen.ayahHadithFourteenSix_4,
    text4: ElmTextDersFourteen.elmTextFourteenSix_4,
    ayah5: ElmTextDersFourteen.ayahHadithFourteenSix_5,
    text5: ElmTextDersFourteen.elmTextFourteenSix_5,
    ayah6: ElmTextDersFourteen.ayahHadithFourteenSix_6,
    text6: ElmTextDersFourteen.elmTextFourteenSix_7,
  ),
  // page 7
  ElmModel(
    ayah: ElmTextDersFourteen.ayahHadithFourteenSeven_2,
    subtitle: ElmTextDersFourteen.subtitleFourteenSeven_1,
    text: ElmTextDersFourteen.elmTextFourteenSeven_3,
    ayah2: ElmTextDersFourteen.ayahHadithFourteenSeven_3,
    ayah3: ElmTextDersFourteen.ayahHadithFourteenSeven_1,
    text3: ElmTextDersFourteen.elmTextFourteenSeven_4,
  ),
  // page 8
  ElmModel(
    text: ElmTextDersFourteen.elmTextFourteenEight_1,
    ayah: ElmTextDersFourteen.ayahHadithFourteenEight_1,
    text2: ElmTextDersFourteen.elmTextFourteenEight_2,
  ),
  // page 9
  ElmModel(
    title: ElmTextDersFourteen.titleFourteenNine_1,
    ayah: ElmTextDersFourteen.ayahHadithFourteenNine_1,
    text: ElmTextDersFourteen.elmTextFourteenNine_2,
  ),
  // page 10
  ElmModel(
    ayah: ElmTextDersFourteen.ayahHadithFourteenTen_1,
    text: ElmTextDersFourteen.elmTextFourteenTen_1,
    ayah2: ElmTextDersFourteen.ayahHadithFourteenTen_2,
    text2: ElmTextDersFourteen.elmTextFourteenTen_2,
    ayah3: ElmTextDersFourteen.ayahHadithFourteenTen_3,
    text4: ElmTextDersFourteen.elmTextFourteenTen_3,
    ayah4: ElmTextDersFourteen.ayahHadithFourteenTen_4,
    text5: ElmTextDersFourteen.elmTextFourteenTen_4,
  ),
  // page 11
  ElmModel(
    ayah: ElmTextDersFourteen.ayahHadithFourteenEleven_1,
    text: ElmTextDersFourteen.elmTextFourteenEleven_1,
  ),
  // page 12
  ElmModel(
    title: ElmTextDersFourteen.titleFourteenTwelve_1,
    text: ElmTextDersFourteen.elmTextFourteenTwelve_1,
  ),
  // page 13
  ElmModel(
    text: ElmTextDersFourteen.elmTextFourteenTherteen_1,
  ),
  // page 14
  ElmModel(
    ayah: ElmTextDersFourteen.ayahHadithFourteenFourteen_1,
    text: ElmTextDersFourteen.elmTextFourteenFourteen_1,
  ),
  // page 15
  ElmModel(
    ayah: ElmTextDersFourteen.ayahHadithFourteenFifteen_1,
    text: ElmTextDersFourteen.elmTextFourteenFifteen_1,
    ayah2: ElmTextDersFourteen.ayahHadithFourteenFifteen_2,
    text2: ElmTextDersFourteen.elmTextFourteenFifteen_2,
  ),
  // page 16
  ElmModel(
    ayah: ElmTextDersFourteen.ayahHadithFourteenSixteen_1,
    text: ElmTextDersFourteen.elmTextFourteenSixteen_1,
  ),
];
