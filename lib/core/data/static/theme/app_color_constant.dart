import 'package:flutter/material.dart';

class AppColor {
  static const title = Colors.black;
  static const subtitle = Color.fromRGBO(128, 128, 128, 1);
  static const ayahHadith = Color.fromARGB(255, 96, 51, 180);
  static const footer = Color.fromRGBO(158, 158, 158, 1);

  static const floatingColor2 = Color.fromRGBO(240, 236, 225, 0);
  //static const flotingColor = #F2F3E1;
  static final amber = Colors.amber[200];
  static const tail = Colors.teal;

  static const primaryColorBlack2 = Colors.black87;
  static const primaryColorBlack = Colors.black;
  static const transparent = Colors.transparent;
  static const black = Color(0xff000000);
  static const primaryColorGolden = Color.fromRGBO(255, 224, 130, 1);
  static const grey = Color(0xFF8E8E8E);
  //static const grey2 = Color.fromARGB(255, 66, 66, 66);
  //static const primaryColor2 = Color.fromARGB(0, 255, 142, 49);
  // static const primaryColor3 = Color.fromARGB(255, 189, 145, 38);
  // static const primaryColor4 = Color.fromRGBO(255, 224, 130, 1);
  // static const bookYellowPage = Color.fromARGB(255, 255, 249, 196);
  static const Color backgroundcolor = Color(0xffF8F9FD);
}
