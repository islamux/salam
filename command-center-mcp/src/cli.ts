...
function parseArgs(args: string[]): { command: string; options: Record<string, string>; positional: string[] } {
  const result: { command: string; options: Record<string, string>; positional: string[] } = {
    command: '',
    options: {},
    positional: []
  };
  ...
  try {
    ...
    switch (command) {
      case 'get-task-context':
        if (positional.length < 1) throw new Error('Missing task_id');
      ...
      case 'get-milestone-overview':
        if (positional.length < 1) throw new Error('Missing milestone_id');
      ...
      case 'list-tasks':
        result = list_tasks(
          ...
          options.status as any || undefined,
      ...
      case 'get-activity-feed':
        result = get_activity_feed(
          ...
          options.limit ? parseInt(options.limit) : 30
        );
        break;
      ...
      case 'complete-task':
        if (positional.length < 2) throw new Error('Missing task_id or summary');
      ...
      case 'reject-task':
        if (positional.length < 2) throw new Error('Missing task_id or feedback');
      ...
      case 'block-task':
        if (positional.length < 2) throw new Error('Missing task_id or reason');
      ...
      case 'update-task':
        ...
        result = update_task(
          positional[0],
          options.priority as any || undefined,
          options.assignee || undefined,
          options['execution-mode'] as any || undefined,
      ...
      case 'log-action':
        if (positional.length < 3) throw new Error('Missing target_id, action, or description');
        result = log_action(
          positional[0],
          positional[1],
          positional[2],
      ...
      case 'enrich-task':
        ...
        result = enrich_task(
          ...
          options['builder-prompt'] || undefined,
          options['acceptance-criteria'] ? options['acceptance-criteria'].split(',') : undefined,
      ...
      case 'add-milestone-note':
        if (positional.length < 2) throw new Error('Missing milestone_id or note');
        result = add_milestone_note(positional[0], positional[1]);
        break;
      
      case 'set-milestone-dates':
        if (positional.length < 1) throw new Error('Missing milestone_id');
        result = set_milestone_dates(
          positional[0],
          options['actual-start'] || undefined,
          options['actual-end'] || undefined
        );
        break;
      
      case 'update-drift':
        if (positional.length < 2) throw new Error('Missing milestone_id or drift_days');
        result = update_drift(positional[0], parseInt(positional[1]));
        break;
      
      case 'create-milestone':
        if (positional.length < 2) throw new Error('Missing id or title');
        result = create_milestone(
          positional[0],
          positional[1],
          options.domain || 'general',
          options.phase || positional[0],
          options['planned-start'] || undefined,
          options['planned-end'] || undefined
        );
        break;
      
      case 'add-milestone-task':
        if (positional.length < 2) throw new Error('Missing milestone_id or label');
        result = add_milestone_task(
          positional[0],
          positional[1],
          options.priority || 'P2',
          options['acceptance-criteria'] ? options['acceptance-criteria'].split(',') : [],
          options.constraints ? options.constraints.split(',') : [],
          options['depends-on'] ? options['depends-on'].split(',') : [],
          options['execution-mode'] || 'agent'
        );
        break;
      
      case 'register-agent':
        if (positional.length < 3) throw new Error('Missing agent_id, name, or type');
        result = register_agent(
          positional[0],
          positional[1],
          positional[2] as any,
          options.permissions ? options.permissions.split(',') : [],
          options.color || '#9B9BAA',
          options['parent-id'] || undefined
        );
        break;
