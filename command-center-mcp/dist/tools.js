...
// READ TOOLS (8 tools)
export function get_task_context(task_id) {
    try {
...
export function get_task_summary(task_id) {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
...
export function get_project_status() {
    try {
        const state = readTracker();
...
export function get_milestone_overview(milestone_id) {
    try {
        const state = readTracker();
...
export function list_tasks(milestone_id, status, domain) {
    try {
        const state = readTracker();
...
export function get_task_history(task_id) {
    try {
        const state = readTracker();
...
export function get_activity_feed(agent_id, limit = 30) {
    try {
        const state = readTracker();
...
export function start_task(task_id, agent_id = 'orchestrator') {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
...
export function complete_task(task_id, summary, agent_id = 'orchestrator') {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
...
export function approve_task(task_id, feedback) {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
...
export function reject_task(task_id, feedback) {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
        ...
        const priorRevisions = state.agent_log.filter(log => log.target_id === task_id && log.action === 'revision_requested').length;
        // Log action
        state.agent_log.push({
            id: `log_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`,
            agent_id: 'operator',
...
export function reset_task(task_id) {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
...
export function block_task(task_id, reason, agent_id = 'orchestrator') {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
...
export function unblock_task(task_id, resolution, agent_id = 'orchestrator') {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
...
export function update_task(task_id, priority, assignee, execution_mode, notes, agent_id = 'orchestrator') {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
...
export function log_action(target_id, action, description, tags = [], agent_id = 'orchestrator') {
    try {
        const state = readTracker();
        // Create log entry
        const logEntry = {
...
export function enrich_task(task_id, prompt, builder_prompt, acceptance_criteria, constraints, context_files, reference_docs, agent_id = 'orchestrator') {
    try {
        const state = readTracker();
        const result = findTask(state, task_id);
        if (!result) {
...
export function add_milestone_note(milestone_id, note) {
    try {
        const state = readTracker();
        const milestone = state.milestones.find(m => m.id === milestone_id);
        if (!milestone) {
...
export function set_milestone_dates(milestone_id, actual_start, actual_end) {
    try {
        const state = readTracker();
        const milestone = state.milestones.find(m => m.id === milestone_id);
        if (!milestone) {
...
export function update_drift(milestone_id, drift_days) {
    try {
        const state = readTracker();
        const milestone = state.milestones.find(m => m.id === milestone_id);
        if (!milestone) {
...
export function create_milestone(id, title, domain = 'general', phase = id, planned_start, planned_end) {
    try {
        const state = readTracker();
        // Check for duplicate
...
export function add_milestone_task(milestone_id, label, priority = 'P2', acceptance_criteria = [], constraints = [], depends_on = [], execution_mode = 'agent') {
    try {
        const state = readTracker();
        const milestone = state.milestones.find(m => m.id === milestone_id);
        if (!milestone) {
...
export function register_agent(agent_id, name, type, permissions, color = '#9B9BAA', parent_id) {
    try {
        const state = readTracker();
